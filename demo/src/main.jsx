      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-12 flex items-start justify-between gap-6">
          <div>
            ...
            <h1>capsolve<span>-sp</span></h1>
            <p className="mt-4 max-w-xl text-lg leading-8 text-[#625b50]">画像はサーバーへ送らず、このブラウザの中だけで5文字を読み取ります。</p>
          </div>
          <a className="github-link" href="https://huggingface.co/nakasyou/capsolve-sp" target="_blank" rel="noreferrer">Model ↗</a>
        </header>
        <section className="grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
          <div className="panel upload-panel">
            ...
            <label className="dropzone" htmlFor="captcha-file">
              {preview ? <img src={preview} alt="選択した CAPTCHA" /> : <><span className="drop-icon">＋</span><strong>画像をドロップ</strong><small>またはクリックして選択</small></>}
              <input id="captcha-file" type="file" accept="image/*" onChange={selectFile} />
            </label>
            <div className="mt-6 flex flex-wrap items-center gap-4">
              <button onClick={predict} disabled={!file || busy}>{busy ? "推論中…" : "読み取る"}</button>
          ...
          <div className="panel result-panel">
            ...
            <p className="result-note">{result ? "5文字すべての予測結果" : "結果がここに表示されます"}</p>
          </div>
        </section>
        <footer className="mt-8 flex flex-wrap justify-between gap-3 text-sm text-[#847a6d]">
          ...
          <a href="https://github.com/nakasyou/capsolve-sp" target="_blank" rel="noreferrer">Source on GitHub ↗</a>
        </footer>
      </div>
    </main>
  );
